<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            @if(Auth::user()->sport =='approved')
            @if(Auth::user()->faculty =='approved')
            @if(Auth::user()->bursar =='approved')
            @if(Auth::user()->library =='approved')
            @if(Auth::user()->department =='approved')
            @if(Auth::user()->security =='approved')
            @if(Auth::user()->hostel =='approved')
            @if(Auth::user()->laboratory =='approved')
            <button type="button"
                class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                onclick="window.print()">Print this pages</button>
            @endif
            @endif
            @endif
            @endif
